...
| LED-01 | Ledger | Toda mutação de `SaldoEstoque` pelos services de estoque gera `MovimentacaoEstoque` na mesma transação. | Helper `_registrar_movimentacao` em todos os services de saldo. | Reserva, liberação, consumo, saída e estorno de saída geram movimentação; saldo nunca muda sem ela. | ADR-0015 |
